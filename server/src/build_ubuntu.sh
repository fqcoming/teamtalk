...
build() {
    ...
    for i in base slog  route_server msg_server http_msg_server file_server  tools db_proxy_server msfs login_server ; do     
        cd $CURPWD/$i
        cmake .
        make
        if [ $? -eq 0 ]; then
...
distclean() {
    # base slog  route_server msg_server http_msg_server file_server tools db_proxy_server msfs login_server
    # base
    echo "distclean base"
    cd base
    make clean
    ...
    echo "distclean test_db_proxy"

    if [ -d "./test_db_proxy/" ];then
        echo "文件夹存在"
        rm -rf test_db_proxy
    else
        echo "文件夹不存在"
    ...
    echo "distclean etcd_login_server"
    if [ -d "./etcd_login_server/" ];then
        echo "文件夹存在"
        rm -rf test_db_proxy
    else
        echo "文件夹不存在"
    ...
    echo "distclean etcd_msg_server"
    if [ -d "./etcd_msg_server/" ];then
        echo "文件夹存在"
        rm -rf etcd_msg_server
    else
        echo "文件夹不存在"
    fi

    echo "distclean protobuf"
    if [ -d "./protobuf/" ];then
        echo "文件夹存在"
        rm -rf protobuf
    else
